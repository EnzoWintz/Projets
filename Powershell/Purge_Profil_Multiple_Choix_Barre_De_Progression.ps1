#Auteur : WINTZ ENZO
#Suppression ou renommage d'un profil avec choix multiple

#Déclaration du multithreading avec la barre de progression
...
#Fonction qui va créer le répertoire de logs dans le C:\TEMP
function create_repo_logs {
$Global:DIR_LOGS = "C:\TEMP\"
$Global:DIR_LOGS2 = "C:\TEMP\LOGS"
if (-not(test-path $Global:DIR_LOGS))
    {
        New-Item -Path $Global:DIR_LOGS -ItemType Directory 
        if (-not(test-path $Global:DIR_LOGS2))
            {
            New-Item -Path $Global:DIR_LOGS2 -ItemType Directory
            }
    }
elseif (-not(test-path $Global:DIR_LOGS2))
       {
        New-Item -Path $Global:DIR_LOGS2 -ItemType Directory
       }
}
...
#On récupère l'ID de session de l'utilisateur connecté
$Global:Date= Get-Date -Format "HH_mm"
$Global:x = GWMI -CL Win32_ComputerSystem
$Global:ID = $($Global:x.UserName).Split("\",7) 
...
PARAM(                     
    ...
    "<![LOG[$Message]LOG]!><time=$([char]34)$date+$($TimeZoneBias.bias)$([char]34) date=$([char]34)$date2$([char]34) component=$([char]34)$component$([char]34) context=$([char]34)$([char]34) type=$([char]34)$severity$([char]34) thread=$([char]34)$([char]34) file=$([char]34)$([char]34)>"| Out-File -FilePath "$Global:DIR_LOGS2\$Global:ID-PurgeProfil.Log" -Append -NoClobber -Encoding default            
    } 

#Fonction de multithread supplémentaire au sein de la barre de progression
...
#Fonction de box qui proposera un choix de réponse
function Input_box {
   param (
   [Parameter(Mandatory=$true)]
    $Input_Box_Option,
...
#Fonction de renommage du profil avec suppression des clés de registre liées au profil de l'utilisateur
...
#Fonction qui va supprimer le profil utilisateur sur le poste sans possibilité de récupérer les données de celui-ci
...
#Fonction qui va supprimer les dossier .old qui ne nécessite plus d'être présent sur le poste
function delete_dir {
Update-Window -Control rename_old_yes -Property ForeGround -Value white
Update-Window -Control delete_symbolic_link -Property ForeGround -Value white 
Update-Window -Control delete_symbolic_link2 -Property ForeGround -Value white
...
#Fonction qui sera réutilisée pour proposer le redémarrage du poste
...
#Fonction qui vérifie que les actions faites ne seront pas appliquées à l'utilisateur actuellement connecté
...
<Window
   ...
   <Grid Margin="10,40,10,10">
      ...
      <TextBlock Grid.Column="0" Grid.Row="1" Margin="5">Matricule :</TextBlock>
...
#On attribuera un filtrage des profil en ignorant volontairement le profil Public et le profil default
